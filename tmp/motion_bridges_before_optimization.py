...
def make_bridge(backend, start, end, lower, upper, speed, acceleration, jerk,
                minimum=0.35, maximum=10.0) -> Bridge:
    ...
    for duration in np.unique(np.r_[initial * 1.2 ** np.arange(40), maximum]):
        if duration > maximum:
            continue
        bridge = HermiteBridge.between(start, end, float(duration))
        lo, hi = bridge.extrema()
        if np.any(lo < lower-1e-7) or np.any(hi > upper+1e-7):
            continue
        if all(_within(*bridge.extrema(k), bound) for k, bound in
               ((1, speed), (2, acceleration), (3, jerk)) if np.any(np.isfinite(bound))):
            return bridge
    raise InfeasibleBridge("No feasible Hermite duration in the search interval")
...
class AuthoredClock:
    ...
    @property
    def weight(self):
        return boundary_weight(self.seconds, self.entry, self.duration, self.window)
    ...
    def advance(self, now, music_rate):
        ...
        while remaining_wall > 1e-12 and self.seconds < self.duration:
            dt = min(remaining_wall, 1/240)
            rate = 1 + self.weight * (max(float(music_rate), 1e-6)-1)
            needed = (self.duration-self.seconds)/rate
            if needed <= dt:
