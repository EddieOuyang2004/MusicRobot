...
framework \cite{bogdanov2013essentia}. Librosa provides a Python implementation
of reusable audio and music-analysis primitives \cite{mcfee2015librosa}.
BeatWeaver uses librosa and NumPy for its DSP descriptors. The Essentia citation
provides library context rather than implementation attribution. Such descriptors are computationally
...
Onset detection supplies evidence of local change, whereas beat tracking seeks
a recurring temporal organisation of those changes. Bello et al. distinguish
energy, spectral and phase-based onset cues, explaining why a single detection
function cannot capture every kind of musical attack \cite{bello2005tutorial}.
This motivates using onset density and spectral flux alongside estimated
tempo. Harmonic/percussive separation supplies another complementary cue.
FitzGerald uses median filtering along time and frequency to distinguish
sustained harmonic structure from transient percussion \cite{fitzgerald2010harmonic}.
BeatWeaver applies the librosa separation primitive to a mel representation
to derive a percussive ratio. That ratio is an engineered activity cue.
...
Grosche and M\"uller's predominant local pulse (PLP) method estimates local
periodicities from onset information and combines them into a pulse function
\cite{grosche2010extracting}. It supplies the algorithmic basis for BeatWeaver's
rolling-window beat candidates through librosa's PLP implementation. Computing
PLP on acquired audio preserves the no-future-input constraint, but local
analysis and asynchronous delivery still introduce delay. An estimated beat
position and the time at which the controller receives it are consequently
different quantities. This distinction motivates the delivered-event metric
and explicit timing logs in Chapter~\ref{chap:experimental_evaluation}.

Learned rhythm analysis offers a complementary direction. The madmom library
includes neural and probabilistic methods for onset, beat, downbeat and metre
analysis \cite{bock2016madmom}. Its distinction between those tasks highlights
a limitation of treating every fourth accepted pulse as a bar boundary.
Counting pulses does not itself estimate metre or downbeats. Madmom is a
literature comparison and a possible future baseline. It is not the beat
backend evaluated in this dissertation.
...
AIST++ reconstructs human motion from the AIST Dance Video Database
\cite{tsuchida2019aist}. The ten genre names and identifier codes used here are
breaking (BR), house (HO), ballet jazz (JB), street jazz (JS), krump (KR),
LA-style hip-hop (LH), locking (LO), middle hip-hop (MH), popping (PO), and
waacking (WA). These are dataset dance categories, not an exhaustive taxonomy
of music. The same codes label the genre retrieval results in
Table~\ref{tab:ch6_genres}.
...
\begin{figure}[htbp]
    ...
    Fig.~1 of Li et al.~\cite{li2021ai}}
...
Distributional evaluation also needs a stated representation and sampling
protocol. Fr\'echet Inception Distance (FID), introduced by Heusel et al.,
compares Gaussian feature distributions \cite{heusel2017gans}. AIST++/FACT and
Bailando adapt this evaluation family to kinetic and geometric motion
features. The corresponding diversity (Div) measures describe distances among
motion feature vectors \cite{li2021ai,siyao2022bailando}. These scores capture
distributional resemblance and spread rather than a unique correct choreography.
Chong and Forsyth show that finite-sample FID estimates can have model-dependent
bias \cite{chong2020effectively}. Thus matching sample counts is necessary but does not
make a small reconstructed retrieval sample a leaderboard-equivalent benchmark.
Chapter~\ref{chap:experimental_evaluation} defines both metrics, their feature
spaces and their sampling and normalisation conventions explicitly.
...
selection. Second, repeated motion patterns are not inherently undesirable, because
...
\begin{figure}[htbp]
    ...
    Fig.~1 of Huang et al.~\cite{huang2024beat}}
...
\section{Real-Time Audio-Driven Control}
...
\begin{equation}
    (i_t,\phi_t,\mathbf{q}^{\mathrm{out}}_t)
    = \mathcal{G}\!\left(\mathcal{A}_{\leq t-\delta},
                         \mathcal{M},\mathcal{H}_t\right),
\end{equation}

where $\delta$ represents acquisition and processing delay, $\mathcal{M}$ is
the available motion collection, and $\mathcal{H}_t$ is controller history,
including previously emitted configurations $\mathbf{q}^{\mathrm{out}}_{<t}$.
The outputs are the selected motion index, phase and emitted configuration.
Here $\mathcal{G}$ denotes online control, while $\mathcal{F}$ above denotes
feature extraction. A strictly causal
...
poses $\mathbf{q}^{\mathrm{out}}_{<t}$ are committed. An algorithm may update its internal
...
beyond a fixed set of stored trajectories. The BeatWeaver assumes that a library
...
The BeatWeaver uses the term \emph{Weaver} in a related but more
...
yaw jump. Joint interpolation should also respect angular topology and take the
...
problem. The motion should instead receive a lower compatibility score during
selection.
...
Gleicher's retargeting formulation treats changes of character proportions as
a constrained motion-editing problem, preserving features of the source
performance while adapting it to a different body \cite{gleicher1998retargetting}.
This provides a foundation for the embodiment problem. Source pose similarity
alone does not preserve a world-space contact when limb lengths change.
For a robot, joint ranges and mechanical geometry add further constraints,
which motivates separate fidelity and feasibility checks.
...
DeepMimic demonstrates a different use of motion examples. Reinforcement
learning trains a controller to imitate reference skills through physical
simulation \cite{peng2018deepmimic}. The example trajectory guides behaviour,
while the learned policy must produce actions under simulated dynamics. This
distinguishes a reference library from an executable controller. BeatWeaver
constructs and times robot references but does not train such a dynamics-based
tracking policy, so its kinematic evaluation cannot establish the balance or
disturbance-recovery capabilities studied in physics-based imitation.
