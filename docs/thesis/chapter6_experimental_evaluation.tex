...
F-full: 60 Hz, 40 s & 13 inputs; full & 0--4 & 65 \\
F-authored: 60 Hz, 40 s & Same 13 inputs; authored & 0--4 & 65 \\
F-long: 60 Hz, 606 s & 1 stitched input; full & 0--4 & 5 \\
\bottomrule
\end{tabular}
...
Only the final three rows, identified by the F- prefix, constitute the 135
formal runs. Historical runtime suites are documented separately in
Appendix~\ref{app:experiment_provenance}.}
...
\subsection{Experiment provenance and scope of comparison}

This chapter distinguishes offline characterisation, historical runtime
experiments, the capacity pilot and the final formal runtime cohort.
The historical experiments used preanalysed musical timing and included a
350-run full-system replay suite and a separate 520-run component-ablation
suite. They are retained as development evidence, with their configurations
and former row names documented in Appendix~\ref{app:experiment_provenance}.
Their runtime scores are excluded from the result tables in this chapter and
from the 135-run formal denominator. The historical ablation uses its own
65-run full-system control, separate from the 350-run replay suite.

All runtime result tables here use the row identifiers F-full, F-authored and
F-long from Table~\ref{tab:ch6_protocol_matrix}. These denote, respectively,
the final 60 Hz causal short full-system, short authored-timing and long
full-system groups. Figure labels ``Causal full'', ``Causal authored'' and
``Causal long'' refer to these same three groups. Only F-full versus F-authored
is a matched formal ablation; F-long evaluates sustained operation, and the
120 Hz pilot is a separate capacity test. Reusing a condition name such as
``full'' or ``authored'' across versions does not make the runs interchangeable.

The offline retrieval/rejection results and the 200 reconstructed SMPL outputs
remain separate evidence with the units given in Table~\ref{tab:ch6_protocol_matrix}.
The latter reuse historical replay traces for motion-distribution analysis;
they do not add runtime runs to the formal cohort or support online timing
claims. The appendix identifies the archived and final result directories so
that a result's provenance does not depend on its condition name alone.

\section{Evaluation Metrics}

\subsection{Retrieval, rejection and statistical units}

Catalogue matching is measured by genre Recall@$K$, macro-$F_1$, mean
reciprocal rank (MRR) and NDCG@5. Source motion recall is retained only as an internal
...
\caption{Source-averaged rhythm and adapted physical-contact scores for the
60 Hz formal cohort. Row IDs follow Table~\ref{tab:ch6_protocol_matrix};
...
\caption{Post-warm-up final-output diagnostics for the 60 Hz formal cohort.
Row IDs follow Table~\ref{tab:ch6_protocol_matrix}.}
...
\caption{60 Hz formal cohort: medians across runs of each run's query p95, control-work p99 and
...
\caption{Stage-resolved timing for F-full (60 Hz formal short full-system group)}
...
\caption{Process-start measurements for F-full (60 Hz formal short full-system group)}
...
\caption{Whole-run operational readiness counters for the 60 Hz formal cohort.
Row IDs follow Table~\ref{tab:ch6_protocol_matrix}.}
...
The formal ablation compares F-full with F-authored on the same 13 inputs
and five matched seeds at 60 Hz. Authored timing disables
...
search or output limiting; those component benefits remain unestablished by
the formal cohort. The historical Legacy retrieval, No diversity, Fixed entry
and Replay authored conditions belong to the separate replay ablation described
in Appendix~\ref{app:experiment_provenance}; they are not additional arms of
this formal comparison.

\begin{table}[tb]
\centering\small
\input{experiment_results/reanalysis_rescue_60_v5/chapter6_tables/ablation.tex}
\caption{Formal 60 Hz timing ablation after source-level seed averaging.
Both rows compare F-authored with F-full; $\Delta$ is F-authored minus F-full.}
