...
The BeatWeaver can make a reliable decision only if its candidate
...
coefficients (MFCCs), and spectral contrast. These primitives are implemented
with librosa and NumPy, including librosa's dynamic-programming beat tracker
\cite{mcfee2015librosa,ellis2007beat}. The percussive ratio uses librosa's
median-filter harmonic/percussive separation on the mel representation
\cite{fitzgerald2010harmonic}. This is a task-specific descriptor aggregation;
Essentia is not the DSP feature-extraction library used here.
...
\begin{figure}[htbp]
    ...
    \texttt{gBR\_sBM\_cAll\_d04\_mBR0\_ch01}.}
...
$\mathbf{Q}=[Q_w,Q_x,Q_y,Q_z]$. Because $\mathbf{Q}$ and $-\mathbf{Q}$ encode the same
