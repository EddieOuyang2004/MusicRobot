...
The emphasis of the chapter is data correctness rather than online behaviour.
Audio-to-motion ranking, switching hysteresis, beat acceptance, and transition
playback are discussed in the following chapter. Here, the important question
is more fundamental: what evidence must be established before an item is safe
to expose to the real-time matcher?
...
may be aggregated at track level. The current catalogue contains 70 distinct
audio variants and 348 analysis segments drawn from 60 music identifiers.
...
contract. A change of model weights is consequently a catalogue migration,
not a transparent implementation detail: existing reference vectors must be
...
deviations are guarded before standardisation. The JSON and NPZ paths are tied
together by catalogue metadata so that a stale array file cannot be interpreted
as belonging to a newly generated index.
...
not presented as universal properties of dance. Their purpose is to support
...
MuJoCo is used here as a consistent model evaluator rather than as evidence of
dynamic stability \cite{todorov2012mujoco}. Where shallow contact behaviour
differs between MuJoCo versions, the validator uses explicit geometry-distance
queries so that the intended separation contract remains visible.
...
clip-to-clip blending are therefore addressed during playback, not baked into
the canonical data.
...
Version fields have operational meaning. The current direct SMPL-to-G1 format
uses artefact format version~1 and pipeline version~4. It explicitly rejects
older provenance paths, including intermediate BVH conversions, so that a
legacy file cannot be mixed silently with trajectories created from direct
...
not a log of attempted work. They also make interrupted builds recoverable: the
...
their role is to detect malformed data rather than tune motion style.
...
Preflight is therefore a kinematic admission test, not a claim that the robot
can dynamically track the trajectory. It catches schema mismatch, model-order
...
The current catalogue has schema version~2 and contains 411 paired manifest
...
$1.31\,\mathrm{m\,s^{-1}}$. These figures document the current artefact rather
than define fixed system constants; a rebuild with different data, model
...
one learned-model identity. Second, dense matrices have known dimensions and
normalisation statistics. Third, every selectable motion has stable source and
retargeting provenance. Fourth, key poses and activity statistics have been
computed consistently from the source sequence. Fifth, the exact trajectory
can be loaded, grounded, and evaluated in the target MuJoCo kinematic model.

These guarantees intentionally stop short of several stronger claims. A
Discogs-derived embedding is not a direct measure of choreographic correctness.
A velocity valley is not necessarily a semantic dance boundary. IK and
self-collision constraints do not prove balance, torque feasibility, or safe
hardware execution. Finally, AIST++ pairings reflect the choreography in the
dataset and may not cover arbitrary live music or all desired G1 behaviours.
The online matcher must therefore combine several weak signals, expose
confidence and rejection reasons, and fall back conservatively when evidence is
poor. The experimental chapters should evaluate retrieval relevance,
transition quality, timing, and robot feasibility separately rather than
compressing them into one success rate.

\section{Chapter Summary}
\label{sec:offline_summary}

This chapter detailed the construction of the Humanoid Matcher's offline
knowledge base. Music is deduplicated, divided into overlapping windows, and
encoded by a fixed Discogs EffNet model together with explicit rhythm and
timbre descriptors. Human motion is analysed for low-velocity key poses,
converted from SMPL coordinates, retargeted to the 29-DoF Unitree G1 through
collision-aware GMR, and conditioned for temporal continuity. Versioned
artefacts, cryptographic provenance, validated caching, quarantine, and atomic
manifests make the build reproducible and recoverable. Finally, a headless
MuJoCo preflight establishes kinematic catalogue eligibility before audio and
motion records are joined. The resulting catalogue supplies the consistent
reference data assumed by the online matching and control methods developed in
the next chapter.
