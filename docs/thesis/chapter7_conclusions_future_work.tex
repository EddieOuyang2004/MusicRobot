...
The resulting BeatWeaver connects offline AIST++ preparation and G1
...
operating point, while leaving the 120 Hz design target in
Section~\ref{sec:intro_problem} unmet. This chapter summarises the
...
Chapter~6 obtained genre Recall@1 of \ChSixRecallOne{} and Recall@5 of
\ChSixRecallFive{}. Thus the
...
is a contribution of the work, but the formal experiment isolates only the
full versus authored-timing comparison. It does not establish a separate
benefit for retrieval scoring, selection policy or entry search. In particular, a diversity benefit cannot be inferred merely from
...
suite, harmonic beat alignment score (BAS) was \ChSixAuthoredBas{} under
authored timing and \ChSixFullBas{} with the full controller. The
source-averaged gain was \ChSixBasGain{}, with a
95\% bootstrap interval of [\ChSixBasGainLow{}, \ChSixBasGainHigh{}] and a
Holm-adjusted $p$-value of
\ChSixBasHolm{}. The formal experiment therefore does not establish a timing benefit
under the specified delivered-beat metric.
...
final poses and actual output clocks. The formal matrix in
Table~\ref{tab:ch6_protocol_matrix} contains 130 matched short runs and five
long runs. All 135 pass the registered real-time, readiness and final-
...
\ChSixFullWork{} ms. The five long runs have a median of \ChSixLongWork{} ms.
All 135 formal runs
...
The BeatWeaver adapts existing clips and cannot supply a movement vocabulary
...
Recall@1 ranged from 1.000 for house (HO) to 0.000 for ballet jazz (JB), street jazz (JS) and
krump (KR), using the genre key in Table~\ref{tab:ch6_genres}, in the six-identities-per-genre evaluation. Broad Discogs-to-AIST++ label
...
causal long runs repeat only one stitched source. The formal ablation covers
...
subordinate to coherent musical phrasing.
...
measured final-output contract. The BeatWeaver would supply motion references
