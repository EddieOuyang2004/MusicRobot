...
% No Chapter 5 reference entries were added to references.bib.
...
Let $x_{0:t}$ denote audio observed up to current time $t$, and let
...
The online system produces a piecewise trajectory
$\widehat{M}(t)$ and its continuous phase $\phi(t)$ using only $x_{0:t}$:

\begin{equation}
    \bigl(\widehat{M}(t),\phi(t)\bigr)
    =\mathcal{F}\!\left(x_{0:t},\mathcal{M},\mathcal{H}_{t}\right),
...
$\widehat{M}(t)$ oscillate when several catalogue items have similar scores.
...
\begin{aligned}
    ...
    s_{\mathrm{music}}(i)&\geq s_{\mathrm{music}}(i^{*})-0.08.
\end{aligned}
...
Beat candidates are produced from a rolling-window predominant-local-pulse
analysis, while block-level onset and loudness histories supply local event
contrast. Dynamic beat tracking and event salience are established tools for
relating musical time to control \cite{ellis2007beat}. In the present method,
...
references, and motion identifier are committed together. Only after this transfer does, the selection policy reset its held-bar count and record
